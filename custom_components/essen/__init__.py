...
Neue Features gegenüber Original (ThomyieD/Essensplaner-HA):
  ...
  - Services: abendessen_hinzufuegen, abendessen_entfernen,
              reste_hinzufuegen, reste_entfernen, reste_portionen_aendern,
              reste_alle_laden
...
class MealPlanner:
    ...
    def __init__(self, base_path: str) -> None:
        ...
        self.reste_file = self.base_path / "reste.json"
        ...
        self.public_reste_file = self.base_path.parent / "www" / "essen-reste.json"
        self.public_card_file = self.base_path.parent / "www" / CARD_FILENAME

        self.default_dishes_file = Path(__file__).with_name("default_gerichte.json")
        self.card_source_file = Path(__file__).with_name("frontend") / CARD_FILENAME
    ...
    def ensure_files(self) -> None:
        with self._lock:
            ...
            self._publish_reste_data(self._load_reste())
    ...
    def reroll_day(self, day: str, year: int | None = None, week: int | None = None) -> dict[str, Any]:
        with self._lock:
    ...
    def clear_day(self, day: str, year: int | None = None, week: int | None = None) -> dict[str, Any]:
        with self._lock:
            plan = self._get_or_create_plan(year, week)
    ...
    def reste_hinzufuegen(
        ...
        if ablauf_datum is None:
            tage = RESTE_ABLAUF_TAGE_EINGEFROREN if _normalize(ort) == _normalize("Eingefroren") else RESTE_ABLAUF_TAGE_KUEHLSCHRANK
    ...
    def reste_summary(self) -> dict[str, Any]:
        with self._lock:
            reste = self._load_reste()
            today = date.today()
            ablaufend_heute = [r for r in reste if r.get("ablauf_datum") == today.isoformat()]
            abgelaufen = [r for r in reste if r.get("ablauf_datum", "9999") < today.isoformat()]
            return {
    ...
    def _resolve_year_week(self, year: int | None = None, week: int | None = None) -> tuple[int, int]:
    ...
    def _dish_allowed(self, dish: dict[str, Any], day_key: str, plan: dict[str, Any]) -> bool:
        dish_id = int(dish["id"])
        used_ids = {
            int(day["dish_id"])
            for day in plan["days"]
    ...
    def _load_reste(self) -> list[dict[str, Any]]:
        if not self.reste_file.exists():
            return []
        data = self._read_json_file(self.reste_file)
    ...
    def _load_dishes_data(self) -> dict[str, Any]:
        data = self._read_json_file(self.dishes_file)
    ...
    def _load_plans(self) -> dict[str, Any]:
        data = self._read_json_file(self.plans_file)
        if not isinstance(data, dict):
            return {"version": 1, "current_plan": None, "plans": {}}
        data.setdefault("plans", {})
        data.setdefault("current_plan", None)
        return data
...
async def async_repair_frontend_resource(hass: HomeAssistant) -> dict[str, Any]:
    ...
    if primary is None:
        primary = await resources.async_create_item({"res_type": "module", "url": CARD_RESOURCE_URL})
        changed = True
    elif primary.get("url") != CARD_RESOURCE_URL or primary.get("type") != "module":
        await resources.async_update_item(primary["id"], {"res_type": "module", "url": CARD_RESOURCE_URL})
        changed = True
...
def async_register_services(hass: HomeAssistant, manager: MealPlanner) -> None:
    ...
    # Original-Services
    hass.services.async_register(DOMAIN, "create_plan", service_handler("create_plan"), schema=vol.Schema(YEAR_WEEK_SCHEMA))
    hass.services.async_register(DOMAIN, "reroll_day", service_handler("reroll_day"), schema=vol.Schema({vol.Required("day"): cv.string, **YEAR_WEEK_SCHEMA}))
    hass.services.async_register(
        DOMAIN,
        "set_day",
        service_handler("set_day"),
        schema=vol.Schema({vol.Required("day"): cv.string, vol.Optional("dish_id"): vol.Coerce(int), vol.Optional("dish_name"): cv.string, **YEAR_WEEK_SCHEMA}),
    )
    hass.services.async_register(DOMAIN, "clear_day", service_handler("clear_day"), schema=vol.Schema({vol.Required("day"): cv.string, **YEAR_WEEK_SCHEMA}))
    hass.services.async_register(
        DOMAIN,
        "add_dish",
        service_handler("add_dish"),
        schema=vol.Schema({vol.Required("name"): cv.string, vol.Required("klasse"): KLASSE, vol.Optional("typ", default="Mittag"): cv.string}),
    )
    hass.services.async_register(
        DOMAIN,
        "update_dish",
        service_handler("update_dish"),
        schema=vol.Schema({vol.Required("dish_id"): vol.Coerce(int), vol.Optional("name"): cv.string, vol.Optional("klasse"): KLASSE, vol.Optional("active"): cv.boolean, vol.Optional("typ"): cv.string}),
    )
    hass.services.async_register(DOMAIN, "deactivate_dish", service_handler("deactivate_dish"), schema=vol.Schema({vol.Required("dish_id"): vol.Coerce(int)}))

    # Abendessen
    hass.services.async_register(DOMAIN, "abendessen_hinzufuegen", service_handler("abendessen_hinzufuegen"), schema=vol.Schema({vol.Required("gericht_name"): cv.string, **YEAR_WEEK_SCHEMA}))
    hass.services.async_register(DOMAIN, "abendessen_entfernen", service_handler("abendessen_entfernen"), schema=vol.Schema({vol.Required("gericht_name"): cv.string, **YEAR_WEEK_SCHEMA}))
    ...
    hass.services.async_register(
        ...
        schema=vol.Schema({vol.Required("gericht_name"): cv.string, vol.Optional("portionen", default="1"): cv.string, vol.Optional("ort", default="Kuehlschrank"): cv.string, vol.Optional("datum_gekocht"): cv.string, vol.Optional("ablauf_datum"): cv.string}),
    )
    hass.services.async_register(DOMAIN, "reste_entfernen", service_handler("reste_entfernen"), schema=vol.Schema({vol.Required("reste_id"): cv.string}))
    hass.services.async_register(DOMAIN, "reste_portionen_aendern", service_handler("reste_portionen_aendern"), schema=vol.Schema({vol.Required("reste_id"): cv.string, vol.Required("delta"): vol.Coerce(float)}))
    hass.services.async_register(DOMAIN, "reste_alle_laden", service_handler("reste_alle_laden"), schema=vol.Schema({}))
